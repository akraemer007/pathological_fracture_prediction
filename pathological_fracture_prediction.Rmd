...
Let's look at all of the other continuous variables:
```{r}
non_binary_vars
...
train_test_reset <- function(partition, df) {
  ...
  # non_bin <- df %>%
  #   dplyr::select(one_of(non_binary_vars))
...
## sixth approach: Balance the dataset and try a logit model

Since there are so few positive cases, I'm going to take my test sample and then break it down to a 1:1, 1:2, or 1:3 postitive to negative ratio. This will allow me to reduce training time, and, hopefully, make more accurate models.
...
```{r}
run_nb  <- function(df, partition) {
  train_test <- train_test_reset(partition, df)

  train <- balance_df(train_test$train)
  ...
  # summary(model_nb_6)
  # print(model_nb_1)
  prediction <- predict(model, train_test$test)
...
## Attempt eight logit regression

This took me a bit longer to figure out. Logit models just give probabilities, rather than specific outputs, so it took me a bit to figure out that I had to convert the probabilities 
```{r}
train_test <- train_test_reset(.5, df)
train <- balance_df(train_test$train)
# skimr::skim(train) %>% as_tibble %>% arrange(n_unique)
valid_cols <- train %>% 
...
set.seed(1991)
model_l_8 <- glm(result~., family=binomial(link='logit'), 
                 data=train)

summary(model_l_8)

prediction_l_8 <- predict(model_l_8, train_test$test, type='response')
str(prediction_l_8)

test$prediction <- prediction_l_8
summary(prediction_l_8)

ggplot(test, aes(prediction)) +
  geom_histogram()

test %>% 
  mutate(prediction = if_else(prediction <.5, 'no', 'yes'),
...
## Attempt 9 logit regression remove lots of unhelpful variables
...
most_varied <- train %>%
  dplyr::select(-result) %>%
  map(table, train$result) %>%
  map(prop.table) %>%
  map_dbl(min) %>% 
  enframe %>% 
  arrange(desc(value))

sliced_varied <- most_varied %>% 
  slice(1:138) %>% 
  pull(name)

train %<>% 
  dplyr::select(result, one_of(sliced_varied))

set.seed(1991)
model_l_9 <- glm(result~., family=binomial(link='logit'), 
                 data=train)

summary(model_l_9)

prediction_l_9 <- predict(model_l_9, train_test$test, type='response')

test$prediction <- prediction_l_9
...
test %>% 
  mutate(prediction = if_else(prediction <.1, 'no', 'yes'),
         check = (result == prediction)) %>% 
  count()
```

## Attempt 9 logit regression remove lots of unhelpful variables
extract statistically significant pvalues of coeff, used model 8 since it has more variables

```{r}
signif_vars_only <- coef(summary(model_l_9))[,4] %>%  # 
  enframe %>% 
  filter(value <= .05) %>%
  slice(-1) %>% 
...
train %<>% 
  dplyr::select(result,one_of(signif_vars_only))

model_l_10 <- glm(result~., family=binomial(link='logit'), 
                 data=train)

summary(model_l_10)

prediction_l_10 <- predict(model_l_10, train_test$test, type='response')

test$prediction <- prediction_l_10
...
train %<>% 
  dplyr::select(result, non_binary_vars, one_of(signif_vars_only))
