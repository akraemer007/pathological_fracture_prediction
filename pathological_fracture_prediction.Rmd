...
I examined the variables in a number of ways not shown in this report. I omitted them because my general exploration didn't have many variables that popped out. The continuous variables seem evenly distributed between fracture and non-frature patients. I tried combining and probing the conditions for a long time, but was not able to extract any clear signals myself.
...
Yes, previous model met the requirements, but Naive Bayes can be a bit of a black box. A logit model is a bit easier to interperet, and, in this case, runs significantly faster. 
...
Above we can see `r ordered_variables %>% slice(2:4) %>% pull(term)` are most significant variables. Looking the codes up on the [NCI Thesaurus](https://ncit.nci.nih.gov/ncitbrowser/pages/home.jsf?version=17.04d) we find that the most significant conditions were: general osteoperosis, rehabilitation, and general falls. These variables make sense as osteoporosis is a bone disease, while falls and rehab are often associated with breaking bones.
